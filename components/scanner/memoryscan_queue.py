...
dropList = [
    ...
    [os.path.join('resources','FastIR_x64.tar.gz'), 'FastIR_x64.tar.gz'],
...
StartCommandList = [
    ...
    'gzip -d -k FastIR_x64.tar.gz',
    'tar xf FastIR_x64.tar',
    'tar xf collecte.tar',
...
def scan(targetObject, IOCObjects, hostConfidential):
    ...
    for command in StartCommandList:
        print(RemCom.execCommand(command, drive))
...
def analyse(resultats_scan, tache):
    ...
    loggingiocscan.info('Begin IOC analysis for host %s' % tache.ip)
    smbreachable = True
