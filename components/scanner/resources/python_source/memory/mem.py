...
class _Memory(object):
    ...
    def _GetProcessModules(self, ProcessID, isPrint):
        ...
        if ret == 0:
            ...
            sys.stderr.write('GetProcessModules() Error on Module32First[%d] with PID : %d' % (errCode, ProcessID))
            sys.stderr.write(win32api.FormatMessage(errCode))
            sys.stderr.flush()
        ...
        while ret:
            if isPrint:
                sys.stderr.write("   executable	 = %s" % me32.szExePath)
                sys.stderr.flush()
        ...
        with open(self.output_dir + '\\' + self.computer_name + '_processes_opened_files.csv', 'wb') as output:
            ...
            while ret:
                try:
                    ...
                    sys.stderr.write('Could not open handle for PID : ' + unicode(pe32.th32ProcessID))
                    sys.stderr.flush()
